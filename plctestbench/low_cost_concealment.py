...
class LowCostConcealment:
    ...
    def process(self, buffer: np.ndarray, is_valid: bool):
        if not is_valid:
            ...
            self._extrapolated_concealment_data = np.zeros(
                (self._extraction_length * self._packet_size, self._n_channels)
            )
            for n in range(self._n_channels):
                ...
                self._extrapolated_concealment_data[:, n] = (
                    self.extrapolate_and_fade_in(self._window[:, n], concealment_data)
                )
            ...
            for n in range(self._n_channels):
                buffer_out[:, n] = self.fade_out(
                    buffer[:, n],
                    self._extrapolated_concealment_data[self._packet_size :, n],
                )
            self._crossfade = False
        else:
            buffer_out = buffer
    ...
    def pre_process(self, buffer: np.ndarray) -> np.ndarray:
        ...
        buffer = signal.filtfilt(
            self._lp_filter, [1], buffer, axis=0, padlen=self._packet_size - 1
        )
    ...
    def zero_crossing_detect(self, buffer: np.ndarray) -> np.ndarray:
        ...
        for i in range(1, len(buffer)):
            zero_crossings[i] = np.logical_xor(
                np.signbit(buffer[i]), np.signbit(buffer[i - 1])
            )
            if zero_crossings[i]:
                if (
                    len(zero_crossing_idx) == 0
                    or (i - zero_crossing_idx[-1]) >= self._lower_bound
                ):
    ...
    def extract(self, buffer: np.ndarray, zero_crossings: np.ndarray):
        ...
        linear_series = np.interp(
            np.arange(2 * self._n_m),
            [0, 2 * self._n_m],
            [extracted_periods[-self._n_m + 1], extracted_periods[self._n_m]],
        )
    ...
    def align(self, buffer: np.ndarray, extracted: np.ndarray):
        ...
        zipped = list(
            zip(
                np.abs(extracted_derivative - buffer[-1]),
                np.abs(extracted_derivative - end_slope),
                range(len(extracted_derivative)),
            )
        )
    ...
    def extrapolate_and_fade_in(
        self, buffer: np.ndarray, alligned_concealment: np.ndarray
    ):
        end_slope = buffer[-1] - buffer[-2]
        if end_slope == 0:
            extrapolated = np.repeat(buffer[-1], self._fade_in_length - 1)
        else:
            extrapolated = np.arange(
                buffer[-1], self._fade_in_length * end_slope + buffer[-1], end_slope
            )[1 : self._fade_in_length]
        window = np.arange(0, 1, 1 / self._fade_in_length)[1:]
        alligned_concealment[: self._fade_in_length - 1] = np.multiply(
            alligned_concealment[: self._fade_in_length - 1], window
        ) + np.multiply(extrapolated, 1 - window)
    ...
    def fade_out(self, buffer: np.ndarray, concealment: np.ndarray):
        ...
        ) + np.multiply(concealment[: fade_out_length - 1], 1 - window)
        return buffer
...
def test_pre_process(lcc: LowCostConcealment, file: sf.SoundFile):
    ...
    sf.write(
        "original_tracks/Chonks_filtered_nl.wav",
        filtered_audio_track,
        file.samplerate,
        file.subtype,
        file.endian,
        file.format,
...
def test_zero_crossing_detection(
    ...
    if plot:
        ...
        ax.plot(
            np.arange(len(sampled_filtered_audio)),
            sampled_filtered_audio[:, 0],
            "x",
            markersize=2,
        )
...
def test_fade_out(lcc: LowCostConcealment, file: sf.SoundFile):
    ...
    faded_out_block = lcc.fade_out(
        next_block[:packet_size, 0], faded_in_concealment_tail
    )
    ...
    ax.plot(
        np.arange(lcc._win_size, lcc._win_size + packet_size),
        faded_in_concealment[:packet_size],
    )
