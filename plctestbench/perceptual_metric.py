...
def S1dataset_generateTFmaskfunc(
    center_f, f_axis, ERBspac=1, timespac=0.001, varargin=[8]
):
    ...
    dFi = erb_to_hz(
        freq_to_erb(center_f) + np.arange(ERBmin, ERBmax + ERBspac, ERBspac)
    )
...
def apply_masking_to_cqt(
    cqt_mag, freq_axis, fs, duration, masked_intorno_duration=(150, 150)
):
    ...
    masked_intorno_hops = np.round(
        np.array(masked_intorno_duration) / hop_duration
    ).astype(int)
    cqt_intorno_mag = cqt_mag[
        :,
        int(num_frames / 2 - masked_intorno_hops[0]) : int(
            num_frames / 2 + masked_intorno_hops[1]
        ),
    ]
    ...
    for bin_idx in range(num_bins):
        ...
        dFi, dTi, AMi = S1dataset_generateTFmaskfunc(
            center_f, freq_axis, timespac=1 / fs * hop_length
        )

        kernels = AMi.T[np.newaxis, :, :] * np.ones(
            (num_frames_intorno, len(dFi), len(dTi))
        )
        masker_levels = cqt_intorno_mag[bin_idx, :, np.newaxis, np.newaxis] * np.ones(
            (num_frames_intorno, len(dFi), len(dTi))
        )
        thresholds = masker_levels + kernels
        thresholds = np.pad(
            thresholds,
            ((0, 0), (0, 0), (0, num_frames_intorno)),
            "constant",
            constant_values=-100,
        )
        for frame_idx in range(num_frames_intorno):
            thresholds[frame_idx, :, :] = np.roll(
                thresholds[frame_idx, :, :], frame_idx, axis=1
            )
        ...
        freq_indices = np.where(
            (freq_axis >= np.min(dFi)) & (freq_axis <= np.max(dFi))
        )[0]

        masked_cqt[freq_indices, :] = np.maximum(
            masked_cqt[freq_indices, :], masked_levels[:, :num_frames_intorno]
        )
...
class DCGC(object):
    ...
    def __call__(
        ...
        self.fp1 = (
            asarray(self.cf) + self.c1 * self.ERBwidth * self.b1 / self.order_ERB
        )  # centre frequency of the signal path
    ...
    def control_path(self):
        ...
        self.lct_ERB = 1.5
        self.n_ch_shift = round(
            self.lct_ERB / self.ERBspace
        )  # value of the shift in channels
        # index of the channel of the control path taken from pGc
        self.indch1_control = (
            minimum(
                maximum(1, arange(1, self.nbr_cf + 1) + self.n_ch_shift), self.nbr_cf
            ).astype(int)
            - 1
        )
        ...
        self.pGc_control = RestructureFilterbank(
            self.pGc_control, indexmapping=self.indch1_control
        )
        ...
        self.asym_comp_control = AsymmetricCompensation(
            self.pGc_control, self.fr2_control, b=self.b2, c=self.c2
        )
    ...
    def signal_path(self):
        ...
        class CompensensationFilterUpdater(object):
            ...
            def __call__(self, *input):
                ...
                level1 = maximum(
                    maximum(value1, 0), self.level1_prev * self.dcgc.exp_deca_val
                )
                level2 = maximum(
                    maximum(value2, 0), self.level2_prev * self.dcgc.exp_deca_val
                )
                ...
                level_total = (
                    self.dcgc.lev_weight
                    * self.dcgc.level_ref
                    * (level1 / self.dcgc.level_ref) ** self.dcgc.level_pwr1
                    + (1 - self.dcgc.lev_weight)
                    * self.dcgc.level_ref
                    * (level2 / self.dcgc.level_ref) ** self.dcgc.level_pwr2
                )
                # then it is converted in dB
                level_dB = (
                    20 * log10(maximum(level_total, self.dcgc.level_min))
                    + self.dcgc.RMStoSPL
                )
                ...
                coeffs = asymmetric_compensation_coeffs(
                    self.dcgc.samplerate,
                    fr2,
                    self.target.filt_b,
                    self.target.filt_a,
                    self.dcgc.b2,
        ...
        varyingfilter_signal_path = AsymmetricCompensation(
            self.pGc_signal, self.fr1, b=self.b2, c=self.c2
        )
        ...
        control = ControlFilterbank(
            varyingfilter_signal_path,
    ...
    def __init__(
        ...
        if transform_type == TransformEnum.CQT:
            ...
            self.transform = lambda original, reconstructed: dcgc(
                ...
                Sound(reconstructed, samplerate=self.fs * Hz),
                nbr_cf=self.n_bins,
                min_freq=self.min_frequency * Hz,
                max_freq=self.max_frequency * Hz,
            )
            self.frequency_axis = erbspace(
                self.min_frequency * Hz, self.max_frequency * Hz, self.bins_per_octave
            )
    ...
    def __call__(self, spectrogram):
        if self.linear_mag:
            ...
            spectrogram_reconstructed_residual = (
                spectrogram_reconstructed_mag - spectrogram_original_mag
            )
            ...
            spectrogram_difference = (
                spectrogram["reconstructed"] - spectrogram["original"]
            )
            ...
            freq_axis = librosa.cqt_frequencies(
                spectrogram_original_db.shape[0],
            ...
            if self.masking:
                # lentissimoooo
                mask = (
                    apply_masking_to_cqt(
                        spectrogram_original_db, freq_axis, self.fs, self.intorno_length
                    )
                    + self.masking_offset
                )

                spectrogram_difference_masked = np.where(
                    spectrogram_difference_db < mask, 0, spectrogram_difference
                )
                spectrogram_difference_masked_mag = np.abs(
                    spectrogram_difference_masked
                )
                spectrogram_difference_masked_db = librosa.amplitude_to_db(
                    spectrogram_difference_masked_mag, ref=ref_value
                )
            else:
            ...
            spectrogram_original_loss_db = np.where(
                np.abs(spectrogram_difference_masked) > 0, spectrogram_original_db, 0
            )
            spectrogram_reconstructed_masked_residual = (
                spectrogram_difference_masked_db - spectrogram_original_loss_db
            )
            ...
            if self.metric == "weighted_sum":
                ...
                complement = 100 - avg
                spectrogram_reconstructed_masked_residual = (
                    spectrogram_reconstructed_masked_residual * complement
                )

            spectrogram_reconstructed_masked_residual = np.where(
                spectrogram_reconstructed_masked_residual < 0,
                0,
                spectrogram_reconstructed_masked_residual,
            )
