...
class MongoDatabaseManager(DatabaseManager):
    ...
    def delete_node(self, node_id):
        ...
        filepath = Path(
            database[collection_name].find_one({"_id": node_id})["filepath"]
        )
        if filepath.exists():
            filepath.unlink()
    ...
    def get_child_collection(self, collection_name):
        ...
        node.
        """
        child_collection = self.get_database()[collection_name].find_one(
            {}, {"child_collection": 1}
        )
        return (
            child_collection["child_collection"]
            if "child_collection" in child_collection.keys()
            else None
        )
    ...
    def _check_if_already_initialized(self) -> None:
        ...
        for collection in self.get_database().list_collection_names():
            if (
                self.get_database()[collection].find_one({}, {"child_collection": 1})
                is not None
            ):
...
class TinyDBDatabaseManager(DatabaseManager):
    ...
    def get_database(self, db_name: str = None):
        ...
        if db_name not in self.client:
            with NamedTemporaryFile(
                prefix=f"{db_name}_", suffix=".json", delete=False
            ) as tmp:
    ...
    def delete_node(self, node_id):
        ...
        if child_collection is not None:
            for child in list(
                database[child_collection].search(where("parent") == node_id)
            ):
                self.delete_node(child["_id"])

        filepath = Path(
            database.table(collection_name).get(where("_id") == node_id)["filepath"]
        )
        if filepath.exists():
            filepath.unlink()
    ...
    def get_child_collection(self, collection_name):
        ...
        """
        child_collection = [
            doc["child_collection"]
            for doc in self.get_database().table(collection_name).all()
        ]
        return (
            child_collection["child_collection"]
            if "child_collection" in child_collection.keys()
    ...
    def get_collection(self, node_id):
        ...
        for collection in self.get_database().tables():
            if (
                self.get_database().table(collection).contains(doc_id=node_id)
                is not None
            ):
                return collection
        return None
    ...
    def _check_if_already_initialized(self) -> None:
        ...
        for collection in self.get_database().tables():
            if [
                doc["child_collection"]
                for doc in self.get_database().table(collection).all()
            ] != []:
                self.initialized |= True
