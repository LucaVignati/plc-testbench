...
class ListeningTest(object):
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        if not path.exists(self.webmushra_folder):
            print(
                "Please install webmushra in the following directory: %s",
                self.webmushra_folder,
            )
            return
        ...
        for idx, data in enumerate(stimuli_data):
            ...
            destination.append(
                AudioFile.from_audio_file(
                    reference_file,
                    new_data=stimulus,
                    new_path=test_folder.joinpath(f"{idx}-{index}.wav"),
                )
            )
...
\
      ...
      <p>Please do not change the output volume of the system from now on.</p>",
        ...
        page_data = [
            (self.indexes[i : i + s], self.stimuli[i : i + s])
            for i in range(0, len(self.references), s)
        ]
        for data in page_data:
            ...
            page = {
                ...
                "stimuli": {
                    str(Path(stimulus.get_path()).stem): str(
                        Path(stimulus.get_path()).relative_to(self.webmushra_folder)
                    )
                    for stimulus in stimuli
                },
            }
            page["stimuli"][str(anchor.stem)] = str(
                anchor.relative_to(self.webmushra_folder)
            )
      ...
      <p>Please take 5 minutes of rest from this trial. Take off your headphones and chill for a while.</p>",
        ...
        last_page = {
            ...
            "questionnaire": [
                ...
                {"type": "text", "label": "Surname", "name": "Surname"},
                {
                    "type": "number",
                    "label": "Age",
                    "name": "Age",
        ...
        if config_file_path.exists():
            ...
            if existing_config != config:
                raise ValueError(
                    f"A config file with the same name and different content exists: {config_file_path}. Resolve manually."
                )
        else:
    ...
    def get_results(self) -> list:
                ...
                for _, response_data in test_data.items():
                    for response in response_data["responses"]:
                        stimuli_responses.append(
                            {"id": response["stimulus"], "score": response["score"]}
                        )
        ...
        if stimuli_responses:
            ...
            result = df.groupby("id").agg({"score": ["mean", "std"]}).reset_index()
