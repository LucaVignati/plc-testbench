...
class Settings(object):
    ...
    def from_dict(self, settings_dict):
        ...
        def reconstruct_values(key, value):
            special_chars = list(
                filter(lambda x: x in ["-", "~", "&", "$", "#"], list(str(key)))
            )
        ...
        return (
            new_settings
            if new_settings.keys() == settings_dict.keys()
            else self.from_dict(new_settings)
        )
    ...
    def assert_setting_is_number_greater_than(self, setting_name: str, min_value):
        ...
        if value < min_value:
            raise AssertionError(
                f"{setting_name} should be greater or equal than {min_value}"
            )
    ...
    def assert_setting_is_number_less_than(self, setting_name: str, max_value):
        ...
        if value > max_value:
            raise AssertionError(
                f"{setting_name} should be smaller or equal than {max_value}"
            )

    def assert_setting_is_number_in_range(
        self, setting_name: str, min_value=None, max_value=None
    ):
        try:
...
class BinomialPLSSettings(Settings):
    ...
    def __init__(self, seed: int = 1, packet_size: int = 32, per: float = 0.0001):
        """
...
class LinearCrossfadeSettings(CrossfadeSettings):
    ...
    def __init__(self, length: int = None, type: CrossfadeType = None):
        ...
        """
        super().__init__(
            length=length, function=CrossfadeFunction.power, exponent=1.0, type=type
        )
...
class QuadraticCrossfadeSettings(CrossfadeSettings):
    ...
    def __init__(self, length: int = None, type: CrossfadeType = None):
        ...
        """
        super().__init__(
            length=length, function=CrossfadeFunction.power, exponent=2.0, type=type
        )
...
class CubicCrossfadeSettings(CrossfadeSettings):
    ...
    def __init__(self, length: int = None, type: CrossfadeType = None):
        ...
        """
        super().__init__(
            length=length, function=CrossfadeFunction.power, exponent=3.0, type=type
        )
...
class PLCSettings(Settings):
    ...
    def __init__(
        ...
        super().__init__()
        crossfade = (
            [crossfade] if crossfade and not isinstance(crossfade, list) else crossfade
        )
        fade_in = [fade_in] if fade_in and not isinstance(fade_in, list) else fade_in
        self.settings["crossfade_frequencies"] = (
    ...
    def __validate_frequencies__(self):
        ...
        if len(crossfade_frequencies) > 0:
            assert (
                crossfade_frequencies[0] >= 0
            ), "The first crossfade frequency must be greater than or equal to 0."
            assert all(
                x < y for x, y in zip(crossfade_frequencies, crossfade_frequencies[1:])
            ), "The crossfade frequencies must be in increasing order."
            assert (
                crossfade_frequencies[-1] <= 20000
            ), "The last crossfade frequency must be less than or equal to 20000."

    def __validate__(self):
        self.__validate_frequencies__()
        assert (
            len(self.get("crossfade")) == len(self.get("crossfade_frequencies")) + 1
        ), "The number of crossfade settings must be one more than the number of crossfade frequencies."
    ...
    def set_crossfade_frequencies(self, crossfade_frequencies: List[int]) -> Settings:
        def change_callback(cloned_settings):
            ...
            new_bands_settings = [
                (
                    crossfade_bands[index]
                    if index < len(crossfade_bands)
                    else NoCrossfadeSettings()
                )
                for index in range(0, len(crossfade_frequencies) + 1)
            ]
            cloned_settings.settings["crossfade"] = new_bands_settings

        return self.__change_setting__(
            "crossfade_frequencies", crossfade_frequencies, change_callback
        )
...
class LowCostPLCSettings(PLCSettings):
    ...
    def __validate__(self):
        ...
        assert self.get("max_frequency") > self.get(
            "f_min"
        ), "The maximum frequency must be greater than the minimum frequency."
        assert not (
            self.get("fade_in_length") != 0
            and self.get("fade_in")[0].__class__ != NoCrossfadeSettings
        ), "The built-in fade_in_length cannot be used in conjunction with the general fade_in mechanism."
        assert not (
            self.get("fade_out_length") != 0
            and self.get("crossfade")[0].__class__ != NoCrossfadeSettings
            and len(self.get("crossfade")) == 1
        ), "The built-in fade_out_length cannot be used in conjunction with the general crossfade mechanism."
...
class DeepLearningPLCSettings(PLCSettings):
    ...
    def __init__(
        ...
        self.settings["context_length_samples"] = (
            context_length / 1000.0 * self.settings["fs_dl"]
        )
    ...
    def __validate__(self):
        ...
        assert self.get("lower_edge_hertz") < self.get(
            "upper_edge_hertz"
        ), "The lower edge must be less than the upper edge."
...
class AdvancedPLCSettings(PLCSettings):
    ...
    def __validate__(self):
        ...
        assert (
            keys == {"linked"} or keys == {"left", "right"} or keys == {"mid", "side"}
        ), "The settings must be either linked, left/right or mid/side."
        freq_keys = set(self.get("frequencies").keys())
        assert (
            keys == freq_keys or len(freq_keys) == 0
        ), "If present, the frequencies must be either linked, left/right or mid/side."
        if len(freq_keys) > 0:
            for key in keys:
                assert (
                    len(self.get("frequencies")[key])
                    == len(self.get("settings")[key]) - 1
    ...
    def set_frequencies(self, frequencies: "dict[str, list[int]]") -> Settings:
        def change_callback(cloned_settings):
            ...
            new_channel_settings = {
                ...
                if channel in channel_settings.keys()
            }
            for channel, freqs in frequencies.items():
                if channel not in new_channel_settings.keys():
                    new_channel_settings[channel] = [
                        ZerosPLCSettings() for i in range(0, len(freqs) + 1)
                    ]
                else:
                    new_channel_settings[channel] = [
                        (
                            new_channel_settings[channel][i]
                            if i < len(new_channel_settings[channel])
                            else ZerosPLCSettings()
                        )
                        for i in range(0, len(freqs) + 1)
                    ]
            cloned_settings.settings["settings"] = new_channel_settings
            cloned_settings.settings["stereo_image_processing"] = (
                StereoImageType.dual_mono
                if "left" in new_channel_settings.keys()
                else StereoImageType.mid_side
            )
            cloned_settings.settings["channel_link"] = new_channel_settings.keys() == {
                "linked"
            }

        return self.__change_setting__("frequencies", frequencies, change_callback)

    def set_stereo_image_processing(
        self, stereo_image_processing: StereoImageType
    ) -> Settings:
        def change_callback(cloned_settings):
            ...
            if keys == {"left", "right"}:
                frequencies = {
                    "mid" if k == "left" else "side": v for k, v in frequencies.items()
                }
                channel_settings = {
                    "mid" if k == "left" else "side": v
                    for k, v in channel_settings.items()
                }
            elif keys == {"mid", "side"}:
                frequencies = {
                    "left" if k == "mid" else "right": v for k, v in frequencies.items()
                }
                channel_settings = {
                    "left" if k == "mid" else "right": v
                    for k, v in channel_settings.items()
                }
        ...
        return self.__change_setting__(
            "stereo_image_processing", stereo_image_processing, change_callback
        )
        ...
        def change_callback(cloned_settings):
            ...
            if channel_link:
                ...
                frequencies = {
                    (
                        "left"
                        if stereo_image_processing == StereoImageType.dual_mono
                        else "mid"
                ...
                channel_settings = {
                    ...
                    ): next(iter(channel_settings.values())),
                    (
                        "right"
                        if stereo_image_processing == StereoImageType.dual_mono
                        else "side"
...
class PlotsSettings(Settings):
    ...
    def __init__(self, dpi: int = 300, linewidth: float = 0.2, figsize: int = (12, 6)):
        """
